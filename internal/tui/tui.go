...
// HTTPSReady renders the +1 upgrade prediction for an HTTP backend.
func HTTPSReady(port int) string {
	return lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("#50FA7B")).Render(fmt.Sprintf("✓ https :%d", port+1))
}
